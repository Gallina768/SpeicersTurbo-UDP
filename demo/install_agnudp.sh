	...
	if ! is_hysteria_installed; then
		_is_frash_install=1
						...
						if [[ -n "$_is_frash_install" ]]; then
							...
							echo
							echo -e "$(tbold)CUSTON TUNNEL:$(treset)"
							echo -e "$(tblue)https://t.me/maptechghbot$(treset)"
							echo
							echo -e "Follow me!"
							echo
							echo -e "\t+ Check out my website at $(tblue)https://www.maptech.tech$(treset)"
							echo -e "\t+ Join us on Telegram: $(tblue)https://t.me/maptech_vpn_hub$(treset)"
							echo -e "\t+ Follow me on Telegram: $(tblue)https://t.me/maptechghbot$(treset)"
							echo
							else
								restart_running_services
								start_services
...
perform_remove() {
	...
	echo
	echo -e "$(tbold)Congratulation! MAPTECH-UDP has been successfully removed from your server.$(treset)"
	echo
